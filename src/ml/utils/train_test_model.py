...
class TrainTestModel():
    ...
    def save_model(self, model_fit, results):
        path = os.path.join(self.path_save, self.model_name)
        timestamp = int(datetime.now().timestamp())
        save_path = os.path.join(path, f"model_{timestamp}.pkl")
        results_path = os.path.join(path, f"results_{timestamp}.json")
